...
# pour avoir des plots interactifs
# %matplotlib notebook  
...
def synth_signal(type='sinus', freq=1, Fech=128, Nech=256):
    """To simulate usual signals like sinusoids, periodic rectangular and 
    triangular signals. Input parameters are:
        type = "sinus" (default), "cosinus", rectangle", "triangle"
        freq = fundamental frequency, default is 1.
        Fech = sampling frequency, default is 128.
        Nech = number of samples, default is 256.
        """
    Nech = int(Nech)
    per = 1/freq
    T = (Nech-1)/Fech
    
    t = linspace(0,(Nech-1)/Fech,Nech)
    x = zeros_like(t)
    # x = empty(t.size(), dtype=float)
    if type=="cosinus":
        x = cos(2*pi*freq*t)
    elif type=="sinus":
        x = sin(2*pi*freq*t)
        ...
        k=1
        while k<=2*t[-1]/per+1:
            ind = arange(where(t>=(k-1)*per/2)[0][0],where(t<=k*per/2)[0][-1]+1)
            #ll[k]=size(ind)
            tper2 = t[ind]-(k-1)*per/2 
            x[ind]=(-1)**k * (1-4/per*tper2)
            k=k+1
            #x= sawtooth(freq*t)*square(freq/2*t)    
    elif type=="rectangle":
        x = square(2*pi*freq*t,0.5)
    else :
        print("Choix de signal invalide")
        x="Invalid signal"
        
    return t, x
                
      
def spectre_signal(x, Fech=128, fenetre="boxcar", posfreq=False):
    """ To compute the Fourier spectrum of the signal x sampled at Fech using 
    the analysis window 'fenetre' (boxcar = rectangulaire par défaut).
    Fech = sampling frequency, default is 128.
    fenetre="hann", "boxcar" (default), ...      
    plotstyle = "positive" (default) for positive frequencies only (0<nu<Fe/2), "standard" 
    for all frequencies between 0 and Fech (0<=nu<Fe), "symmetric" for all frequencies centered on 0 (-Fech/2<nu<+Fech/2), or "no" for no plot. 
    """
    f, Sx = periodogram(x, Fech, window=fenetre, return_onesided=posfreq)
            
    return f, Sx # Sx, nu, Phi 


def spectrogramme(x, Fe, window="hann",nperseg=256):

    f, t, Sxx = spectrogram(x, Fe, window=window,nperseg=nperseg,noverlap=round(7*nperseg/8))
...
def fft_segment(k=1):
    ...
    n0=k*500
    N=200
    L = 2*N+1 ;   # nombre d'échantillons considérés par segment
    sel = arange(n0-N,n0+N+1,1)  # indices des échantillons du segment sélectionné
    sigma=100
